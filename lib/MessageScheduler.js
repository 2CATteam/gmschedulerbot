...
class MessageScheduler
{
	...
	scheduleMessage(token, group, time, toSend, messageType)
	{
		...
		var j = schedule.scheduleJob(time, function(w, x, y, z, o, thisnt) {
			sender.sendMessage(w, x, y, z, o);
			thisnt.unscheduleMessage(w, x, y, z);
		}.bind(null, token, group, time, toSend, messageType, this));
		if (j == null)
		{
			sender.sendMessage(token, group, time, toSend, messageType);
			return;
		}
		var job = {
			token: token,

...
class MessageSender
{
	...
	static sendMessage(token, group, toSend, image, messageType, forcedGUID, retrying)
	{
		...
		const guid = forcedGUID ? forcedGUID : (new Date()).getTime().toString() + group.toString() + token.toString().substring(0, 5)
		var body = {
			"message": {
				"source_guid": guid,
				"text": toSend,
		...
		let req = https.request(options,  (response) => {
			...
			if (response.statusCode > 399) {
				console.error("Error sending message, code:", response.statusCode, "with text:", response.statusMessage)
				...
				if (!retrying) {
					MessageSender.sendMessage(token, group, toSend, image, messageType, guid, true)
				}
				return;
			}
		});
			...
			let req = https.request(options,  (response) => {
				...
				response.on('end', () => {
					try {
						...
						console.error("Handling the following error:")
						console.error(e)
						reject(e)
					}
				})
